...
test('container and build context freeze the runtime without shipping local audio or secrets', () => {
  ...
  assert.match(workflow, /Verify runtime identity, surface, and size/);
  assert.match(workflow, /image_size > 251658240/);
  assert.match(workflow, /process\.getuid\(\) === 0/);
  assert.match(workflow, /unexpected \/app runtime surface/);
  assert.match(workflow, /Verify the runtime decoder allowlist/);
  assert.match(workflow, /source\.wav:\/tmp\/source\.wav:ro/);
  assert.match(workflow, /test "\$decoded_bytes" = '88200'/);
});
