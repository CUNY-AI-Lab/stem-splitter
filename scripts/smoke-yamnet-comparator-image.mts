...
const LICENSE_SHA256 = '5b17814bf0de8cf65069bc6d7cc38cff19fcaa864d243423ad3ef3db01b52385';
...
const DEFAULT_MAX_IMAGE_BYTES = 768 * 1024 * 1024;
const SOURCE_FILES = [
...
function mappingIdentity(): MappingIdentity {
  ...
  if (sha256(bytes) !== MAPPING_SHA256) throw new Error('YAMNet mapping bytes drifted');
  ...
  if (
    value.$schema !== 'stem-splitter.yamnet-class-mapping.v1' ||
  ...
  const supportedIds = value.mapped.map((item) => {
    ...
    if (
      ...
      throw new Error('YAMNet mapped item is invalid');
    }
    return item.instrumentId;
  });
  if (supportedIds.length !== 36 || new Set(supportedIds).size !== supportedIds.length) {
    throw new Error('YAMNet supported-label surface drifted');
  }
  if (value.unsupported.length !== 15) throw new Error('YAMNet unsupported-label surface drifted');
  return { supportedIds };
}
...
export function validateSmokeOutput(
  ...
  if (!record(value)) throw new Error('YAMNet smoke output root is invalid');
  exactKeys(
    value,
    [
      '$schema',
  ...
  if (
    ...
    throw new Error('YAMNet smoke output identity does not match');
  }
  const loadMs = integer(value.loadMs, 0, 300_000, 'YAMNet load timing');
  const timingMs = integer(value.timingMs, 0, 300_000, 'YAMNet total timing');
  const window = value.windows[0];
  if (!record(window)) throw new Error('YAMNet smoke output window is invalid');
  exactKeys(
    window,
    ['resampledSamples', 'patches', 'inferenceMs', 'metrics', 'topClasses'],
    'YAMNet smoke output window'
  );
  const resampledSamples = integer(
    window.resampledSamples,
    48_000,
    48_000,
    'YAMNet resampled sample count'
  );
  const patches = integer(window.patches, 1, 32, 'YAMNet patch count');
  const inferenceMs = integer(window.inferenceMs, 0, 300_000, 'YAMNet inference timing');
  if (timingMs + 2 < loadMs + inferenceMs) {
    throw new Error('YAMNet timing accounting is invalid');
  }
  if (!record(window.metrics)) throw new Error('YAMNet smoke metrics are invalid');
  exactKeys(window.metrics, supportedIds, 'YAMNet smoke metrics');
  for (const id of supportedIds) {
    const metric = window.metrics[id];
    if (!record(metric)) throw new Error(`YAMNet smoke metric ${id} is invalid`);
    exactKeys(
      metric,
      ['top3Mean', 'maximum', 'mean', 'patchesAtLeastHalf'],
      `YAMNet smoke metric ${id}`
    );
    ...
    if (maximum < top3Mean || maximum < mean) {
      throw new Error(`YAMNet smoke metric ${id} is inconsistent`);
    }
  }
  if (!Array.isArray(window.topClasses) || window.topClasses.length !== 12) {
    throw new Error('YAMNet smoke top classes are invalid');
  }
  let previousScore = Number.POSITIVE_INFINITY;
  const indexes = new Set<number>();
  for (const item of window.topClasses) {
    if (!record(item)) throw new Error('YAMNet smoke top class is invalid');
    exactKeys(item, ['index', 'mid', 'displayName', 'top3Mean'], 'YAMNet smoke top class');
    const index = integer(item.index, 0, 520, 'YAMNet top class index');
    const currentScore = score(item.top3Mean, 'YAMNet top class score');
    if (
      indexes.has(index) ||
      ...
      throw new Error('YAMNet smoke top class identity is invalid');
    }
    indexes.add(index);
...
function inspectImage(imageReference: string): ImageExecution {
  if (
    ...
    throw new Error('YAMNet smoke image reference is invalid');
  }
  ...
  if (!Array.isArray(document) || document.length !== 1 || !record(document[0])) {
    throw new Error('YAMNet smoke image inspection is invalid');
  }
  ...
  if (
    ...
    throw new Error('YAMNet smoke image runtime surface does not match');
  }
  const platform = `linux/${image.Architecture}` as 'linux/amd64' | 'linux/arm64';
  const expectedPlatform = process.env.YAMNET_COMPARATOR_EXPECTED_PLATFORM;
  if (
    ...
    throw new Error('YAMNET_COMPARATOR_EXPECTED_PLATFORM is invalid');
  }
  if (expectedPlatform && platform !== expectedPlatform) {
    throw new Error('YAMNet smoke image platform does not match the expected platform');
  }
  const maximumBytes = positiveIntegerEnvironment(
    'YAMNET_COMPARATOR_MAX_IMAGE_BYTES',
    DEFAULT_MAX_IMAGE_BYTES,
    1,
    2 * 1024 * 1024 * 1024
  );
  if ((image.Size as number) > maximumBytes) {
    throw new Error('YAMNet smoke image exceeds the size gate');
  }
...
function runImage(
  ...
  containerSequence += 1;
  const containerName = `stem-splitter-yamnet-smoke-${process.pid}-${containerSequence}-${suffix}`;
  const args = [
    'run',
    '--rm',
    '--pull',
...
function readImageFile(
  ...
  const output = successfulOutput(result, 'YAMNet image provenance read');
  if (output.length > maximumBytes) throw new Error('YAMNet image provenance is oversized');
  return output;
}
...
function verifyImageProvenance(execution: ImageExecution): void {
  ...
  if (!/^[a-f0-9]{64}$/.test(lockSha) || lockSha !== repositorySha256(LOCK_PATH)) {
    throw new Error('YAMNet image dependency lock does not match the repository');
  }
  const sourceValue: unknown = JSON.parse(
    readImageFile(execution, SOURCE_MANIFEST_PATH, 16 * 1024).toString('utf8')
  );
  if (!record(sourceValue)) throw new Error('YAMNet image source manifest is invalid');
  exactKeys(sourceValue, SOURCE_FILES, 'YAMNet image source manifest');
  ...
  const surfaceScript = [
    ...
    "roots=[pathlib.Path('/app'),pathlib.Path('/models/yamnet'),pathlib.Path('/opt/yamnet-comparator-provenance')]",
    ...
    "license_sha=hashlib.sha256(pathlib.Path('/models/yamnet/LICENSE.YAMNET').read_bytes()).hexdigest()",
  ...
  const surfaceValue: unknown = JSON.parse(
    successfulOutput(surfaceResult, 'YAMNet runtime-surface check').toString('utf8')
  );
  if (!record(surfaceValue)) throw new Error('YAMNet runtime-surface output is invalid');
  exactKeys(surfaceValue, ['files', 'symlinks', 'licenseSha256'], 'YAMNet runtime surface');
  const expectedFiles = {
    '/app': [
      'backend.py',
      'cli.py',
    ...
    '/opt/yamnet-comparator-provenance': ['source-sha256.json', 'uv-lock.sha256'],
  };
  if (
    ...
    throw new Error('YAMNet runtime surface does not match the pin');
  }
}
...
function assertContractFailure(
  ...
  if (
    ...
    throw new Error(`YAMNet ${suffix} rejection did not match the contract`);
  }
}
...
async function main(): Promise<void> {
  ...
  if (args.length > 1 || (args[0] !== undefined && args[0].startsWith('--'))) {
    throw new Error('usage: smoke-yamnet-comparator-image.mts [IMAGE]');
  }
  const imageReference =
    args[0] || process.env.YAMNET_COMPARATOR_IMAGE || 'stem-splitter-yamnet-comparator:v3.2-candidate';
  ...
  const timeoutMs = positiveIntegerEnvironment(
    'YAMNET_COMPARATOR_SMOKE_TIMEOUT_MS',
  ...
  const output = successfulOutput(result, 'YAMNet constrained synthetic control');
  if (output.length > 64 * 1024) throw new Error('YAMNet smoke output exceeds the contract');
  let outputValue: unknown;
  try {
    outputValue = JSON.parse(output.toString('utf8'));
  } catch {
    throw new Error('YAMNet smoke output is not JSON');
  }
  ...
  assertContractFailure(
    ...
    'PCM sample rate does not match the comparator pin',
    timeoutMs
  );
...
if (import.meta.url === invokedPath) {
  main().catch((error) => {
    process.stderr.write(`${error instanceof Error ? error.message : 'YAMNet image smoke failed'}\n`);
    process.exitCode = 1;
  });
}
