...
import { validateConfiguration, deploymentState, parseRailwayJson } from './railway.mjs';
...
test('malformed CLI responses cannot expose private values in parser errors', () => {
  assert.deepEqual(parseRailwayJson('{"ok":true}'), { ok: true });
  assert.throws(() => parseRailwayJson('{"TOKEN":"private-fixture"'), error => !error.message.includes('private-fixture') && /withheld/.test(error.message));
});
