...
const REPORT_SCHEMA = 'stem-splitter.efficientat-comparator-evaluation.v2';
const OUTPUT_SCHEMA = 'stem-splitter.efficientat-comparator-output.v1';
const MAPPING_SCHEMA = 'stem-splitter.efficientat-class-mapping.v1';
...
export const EFFICIENTAT_EVALUATION_SOURCE_PATHS = Object.freeze({
  evaluator: 'scripts/eval-efficientat-comparator.mts',
  runner: 'scripts/run-efficientat-comparator-eval.sh',
...
export function efficientatEvaluationSourcePins(repositoryRoot = process.cwd()): Record<
  keyof typeof EFFICIENTAT_EVALUATION_SOURCE_PATHS,
  { path: string; sha256: string }
> {
  return Object.fromEntries(
    Object.entries(EFFICIENTAT_EVALUATION_SOURCE_PATHS).map(([name, path]) => [
      name,
...
function assertEvaluationSourcesUnchanged(
  expected: ReturnType<typeof efficientatEvaluationSourcePins>
): void {
  const current = efficientatEvaluationSourcePins();
  for (const name of Object.keys(EFFICIENTAT_EVALUATION_SOURCE_PATHS) as Array<
    keyof typeof EFFICIENTAT_EVALUATION_SOURCE_PATHS
  >) {
    if (
      ...
      throw new Error(`EfficientAT evaluation source changed during execution: ${name}`);
    }
  }
}
...
export function loadMapping(
  ...
  if (sha256Bytes(bytes) !== MAPPING_SHA256) throw new Error('EfficientAT mapping bytes drifted');
  ...
  if (JSON.stringify(value.scoringPolicy) !== JSON.stringify(expectedPolicy)) {
    throw new Error('EfficientAT scoring policy drifted');
  }
  ...
  for (const item of value.mapped) {
    ...
    const instrumentId = normalizedId(item.instrumentId, 'EfficientAT mapped instrument id');
    if (supportedIds.includes(instrumentId) || !Array.isArray(item.classes) || !item.classes.length) {
      throw new Error('EfficientAT mapped instrument is invalid');
    }
    for (const rawClass of item.classes) {
      if (!record(rawClass)) throw new Error('EfficientAT mapped class is invalid');
      exactKeys(rawClass, ['index', 'mid', 'displayName'], 'EfficientAT mapped class');
      ...
      if (
        ...
        throw new Error('EfficientAT mapped class identity is invalid');
      }
  ...
  const unsupported = value.unsupported.map((item) => {
    if (!record(item)) throw new Error('EfficientAT unsupported item is invalid');
    exactKeys(item, ['instrumentId', 'reason'], 'EfficientAT unsupported item');
    const instrumentId = normalizedId(item.instrumentId, 'EfficientAT unsupported instrument id');
    if (typeof item.reason !== 'string' || item.reason.trim().length < 20) {
      throw new Error('EfficientAT unsupported reason is invalid');
    }
  ...
  if (
    ...
    throw new Error('EfficientAT mapping does not partition the vocabulary');
  }
  return {
...
export function secondWindowScore(values: number[]): number {
  if (
    ...
    throw new Error('EfficientAT window scores are invalid');
  }
...
export function validateComparatorOutput(
  ...
  if (!record(value)) throw new Error('EfficientAT output root is invalid');
  exactKeys(
    value,
    [
      '$schema',
  ...
  if (
    ...
    throw new Error('EfficientAT output identity does not match');
  }
  const loadMs = safeInteger(value.loadMs, 0, 300_000, 'EfficientAT load timing');
  const timingMs = safeInteger(value.timingMs, 0, 300_000, 'EfficientAT total timing');
  const windows: ComparatorWindow[] = value.windows.map((window, windowIndex) => {
    if (!record(window)) throw new Error('EfficientAT output window is invalid');
    exactKeys(
      window,
      ['resampledSamples', 'patches', 'inferenceMs', 'metrics', 'topClasses'],
      'EfficientAT output window'
    );
    const resampledSamples = safeInteger(
      window.resampledSamples,
      31_199,
      480_001,
      'EfficientAT resampled samples'
    );
    const patches = safeInteger(window.patches, 1, 1, 'EfficientAT clip count');
    const inferenceMs = safeInteger(window.inferenceMs, 0, 300_000, 'EfficientAT inference timing');
    if (!record(window.metrics) || Object.keys(window.metrics).length !== supportedIds.length) {
      throw new Error('EfficientAT mapped metrics are invalid');
    }
    const metrics: Record<string, WindowMetric> = {};
    for (const id of supportedIds) {
      const metric = window.metrics[id];
      if (!record(metric)) throw new Error(`EfficientAT metric ${id} is missing`);
      exactKeys(
        metric,
        ['top3Mean', 'maximum', 'mean', 'patchesAtLeastHalf'],
        `EfficientAT metric ${id}`
      );
      metrics[id] = {
    ...
    if (!Array.isArray(window.topClasses) || window.topClasses.length !== 12) {
      throw new Error('EfficientAT top AudioSet classes are invalid');
    }
    let previousScore = Number.POSITIVE_INFINITY;
    const seenIndexes = new Set<number>();
    const topClasses = window.topClasses.map((item) => {
      if (!record(item)) throw new Error('EfficientAT top AudioSet class is invalid');
      exactKeys(item, ['index', 'mid', 'displayName', 'top3Mean'], 'EfficientAT top AudioSet class');
      const index = safeInteger(item.index, 0, 526, 'EfficientAT AudioSet class index');
      const score = boundedNumber(item.top3Mean, 0, 1, 'EfficientAT AudioSet class score');
      if (
        ...
        throw new Error('EfficientAT top AudioSet class identity is invalid');
      }
      seenIndexes.add(index);
      previousScore = score;
      return { index, mid: item.mid, displayName: item.displayName, top3Mean: score };
    });
    if (windowIndex >= expectedWindows) throw new Error('EfficientAT returned an extra window');
...
function readImageFile(imageId: string, path: string, maximumBytes: number): Buffer {
  ...
  if (result.status !== 0 || !Buffer.isBuffer(result.stdout) || result.stdout.length > maximumBytes) {
    throw new Error('EfficientAT image provenance is unavailable');
  }
...
export function inspectImage(imageReference: string): ImageExecution {
  if (!imageReference || /[\s\u0000-\u001f\u007f]/.test(imageReference)) {
    throw new Error('EfficientAT image reference is invalid');
  }
  ...
  if (!Array.isArray(document) || document.length !== 1 || !record(document[0])) {
    throw new Error('EfficientAT image inspection is invalid');
  }
  ...
  if (
    ...
    throw new Error('EfficientAT image runtime surface does not match');
  }
  ...
  if (!/^[a-f0-9]{64}$/.test(lockSha256) || lockSha256 !== sha256File(LOCK_PATH)) {
    throw new Error('EfficientAT image dependency lock does not match the worktree');
  }
  const manifestValue: unknown = JSON.parse(
    readImageFile(image.Id, SOURCE_MANIFEST_PATH, 16 * 1024).toString('utf8')
  );
  if (!record(manifestValue)) throw new Error('EfficientAT image source manifest is invalid');
  exactKeys(manifestValue, SOURCE_FILES, 'EfficientAT image source manifest');
...
function comparatorTimeout(emulated: boolean): number {
  const raw = process.env.EFFICIENTAT_COMPARATOR_TIMEOUT_MS;
  if (raw === undefined || raw === '') return emulated ? 180_000 : 60_000;
  if (!/^\d+$/.test(raw)) throw new Error('EFFICIENTAT_COMPARATOR_TIMEOUT_MS is invalid');
  const value = Number(raw);
  if (!Number.isSafeInteger(value) || value < 10_000 || value > 300_000) {
    throw new Error('EFFICIENTAT_COMPARATOR_TIMEOUT_MS is invalid');
  }
  return value;
}
...
export function runComparator(
  ...
  const containerName = `stem-splitter-efficientat-eval-${process.pid}-${slug}`;
  const args = [
    'run',
    '--rm',
    '--pull',
  ...
  try {
    ...
    if (
      ...
      throw new Error(`${slug}: constrained EfficientAT comparator failed`);
    }
    let value: unknown;
    try {
      value = JSON.parse(result.stdout.toString('utf8'));
    } catch {
      throw new Error(`${slug}: EfficientAT comparator did not return JSON`);
    }
...
function parseArguments(args: string[]): {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (argument === '--image' || argument === '--output') {
      ...
      throw new Error(`unknown EfficientAT evaluation flag: ${argument}`);
    } else {
...
async function main(): Promise<void> {
  if (endianness() !== 'LE') throw new Error('EfficientAT evaluation requires a little-endian host');
  const { image, outputPath, slugs } = parseArguments(process.argv.slice(2));
  const evaluationSourcePins = efficientatEvaluationSourcePins();
  ...
  for (const slug of slugs) {
    if (!expectationBySlug.has(slug)) throw new Error(`unknown EfficientAT corpus slug: ${slug}`);
  }
  ...
  const report = {
    ...
    candidate: {
      ...
      officialLicense: 'MIT',
      upstreamRepository: 'fschmid56/EfficientAT',
      upstreamReleaseTag: 'v0.0.1',
      upstreamRevision: '7e30f2bbe85439c15feedd9ba5ad8bff0a600fee',
    },
    execution,
    evaluationSources: {
      ...
      evaluatorPath: 'scripts/eval-efficientat-comparator.mts',
...
if (import.meta.url === invokedPath) {
  main().catch((error) => {
    process.stderr.write(`${error instanceof Error ? error.message : 'EfficientAT evaluation failed'}\n`);
    process.exitCode = 1;
  });
}
