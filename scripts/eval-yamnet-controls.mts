...
const REPORT_SCHEMA = 'stem-splitter.yamnet-control-evaluation.v1';
const EVALUATOR_PATH = 'scripts/eval-yamnet-controls.mts';
const COMPARATOR_EVALUATOR_PATH = 'scripts/eval-yamnet-comparator.mts';
...
export interface YamnetControlObservation {
  id: string;
  instrument: string;
  family: string;
  positiveIds: string[];
...
export function summarizeYamnetControlObservations(
  observations: YamnetControlObservation[],
...
function parseArguments(args: string[]): {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (argument === '--image' || argument === '--output') {
      ...
      throw new Error(`unknown YAMNet control-evaluation flag: ${argument}`);
    } else {
...
async function main(): Promise<void> {
  if (endianness() !== 'LE') throw new Error('YAMNet control evaluation requires a little-endian host');
  ...
  for (const id of ids) {
    if (!knownIds.has(id)) throw new Error(`unknown YAMNet control id: ${id}`);
  }
  ...
  const summary = summarizeYamnetControlObservations(observations);
  const byFamily: Record<string, ReturnType<typeof summarizeYamnetControlObservations>> = {};
  for (const family of [...new Set(observations.map((observation) => observation.family))].sort()) {
    byFamily[family] = summarizeYamnetControlObservations(
...
if (import.meta.url === invokedPath) {
  main().catch((error) => {
    process.stderr.write(
      `${error instanceof Error ? error.message : 'YAMNet control evaluation failed'}\n`
    );
    process.exitCode = 1;
  });
}
