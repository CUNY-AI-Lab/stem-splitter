...
const MAX_PRIVATE_REVIEW_BYTES = 512 * 1024;
...
function parsePaths(args: string[]): { inputPath: string; outputPath: string } {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (argument !== '--input' && argument !== '--output') {
      throw new Error(`unknown NSynth review argument: ${argument}`);
    }
  ...
  if (!inputPath || !outputPath) {
    throw new Error(
      'usage: finalize-nsynth-family-control-review --input <private-review.json> --output <public-review.json>'
    );
  }
...
function readPrivateReview(path: string): { serialized: string; value: unknown } {
  ...
  if (metadata.isSymbolicLink() || !metadata.isFile()) {
    throw new Error('private NSynth review input must be a regular file, not a symbolic link');
  }
  if ((metadata.mode & 0o077) !== 0) {
    throw new Error('private NSynth review input must be owner-only (chmod 600)');
  }
  if (metadata.size < 2 || metadata.size > MAX_PRIVATE_REVIEW_BYTES) {
    throw new Error('private NSynth review input exceeds its 512 KiB safety boundary');
  }
  ...
  try {
    ...
    throw new Error('private NSynth review input is not valid JSON');
  }
  return { serialized, value };
}

export function finalizeNsynthFamilyControlReview(
  inputPath: string,
  outputPath: string,
    ...
    `${JSON.stringify(
      {
        ...
        exactInstrumentClaims: review.claimBoundary.exactInstrumentClaims,
        promotionEligible: review.claimBoundary.promotionEligible,
        blockers: review.claimBoundary.blockers,
      },
      null,
      2
    )}\n`
...
function main(): void {
  ...
  finalizeNsynthFamilyControlReview(inputPath, outputPath);
}
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  try {
    ...
    process.stderr.write(
      `${error instanceof Error ? error.message : 'NSynth family control review finalization failed'}\n`
    );
    process.exitCode = 1;
  }
}
