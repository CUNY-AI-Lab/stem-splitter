...
test('real local receiver: identity, cache, denial, correlation and cancellation', async (t) => {
  ...
  await db.prepare("INSERT INTO jobs (id, filename, source_key, status, stems) VALUES (?, ?, ?, 'done', ?)")
    .bind('fixture', 'Synthetic song', 'source', JSON.stringify([{ name: 'vocals', key: 'vocals.mp3' }, { name: 'instrumental', key: 'instrumental.mp3' }])).run();
  let mode = 'success';
  let registryCalls = 0;
  let providerCalls = 0;
  ...
  const gatewayEnv = {
    ...
    AI: { models: async () => [{ name: MODEL, description: 'Synthetic catalog entry', properties: [{ property_id: 'function_calling', value: 'true' }] }] },
    MODEL_ACCESS_REGISTRY_GATEWAY: {
      resolveAccess: async () => ({ ok: false, code: 'invalid', retryable: false }),
      resolveDoorwayAccess: async () => { registryCalls++; return mode === 'registry-denied' ? { ok: false, code: 'not_admitted', retryable: false } : { ok: true, scope: 'models:invoke models:read quota:read', budgetScope: 'person' }; },
    },
  };
  ...
  try {
    ...
    assert.equal(metadata[0].user_id, TEST_SUBJECTS.alice); assert.equal(metadata[0].budget_scope, 'person');
    ...
    assert.ok(metadata.every(value => value.user_id === TEST_SUBJECTS.alice && value.budget_scope === 'person'));
