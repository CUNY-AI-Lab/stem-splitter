...
test('configuration fails closed without exact release and private verifier configuration', async () => {
 ...
 assert.equal(await fleetConfiguration({ ...env, ASSISTANT_MODEL: 'z-ai/glm-5.2' }), null);
});
test('production Gateway configuration uses the origin because cail-client owns the API path', async () => {
 const production = { ...env, LOCAL_DEV: undefined, CAIL_GATEWAY_URL: 'https://tools.ailab.gc.cuny.edu' };
 assert.notEqual(await fleetConfiguration(production), null);
 for (const suffix of ['/v1', '/v1/', '/?route=other', '/#fragment']) {
  assert.equal(await fleetConfiguration({ ...production, CAIL_GATEWAY_URL: production.CAIL_GATEWAY_URL + suffix }), null);
 }
 assert.equal(await fleetConfiguration({ ...production, CAIL_GATEWAY_URL: env.CAIL_GATEWAY_URL }), null);
 assert.equal(await fleetConfiguration({ ...env, CAIL_GATEWAY_URL: env.CAIL_GATEWAY_URL + '/v1' }), null);
});
