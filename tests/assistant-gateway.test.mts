...
import { AssistantError, COACH_DOWN, gatewayChatStream } from '../src/assistant/gateway.ts';

const env = {
  CAIL_GATEWAY_URL: 'https://gateway.example.test',
  CAIL_GATEWAY_IDENTITY_JWT: 'verified-identity',
  CAIL_CONVERSATION_ID: 'conversation-123',
  ASSISTANT_MODEL: 'vendor/model',
} as Env;
...
test('CAIL client sends verified identity and affinity, drains trailing usage, and assembles fragmented tools', async () => {
  await withFetch(async (url, init) => {
    assert.equal(String(url), 'https://gateway.example.test/v1/chat/completions');
    ...
    assert.equal(reply.requestId, 'request-123');
  });
});
...
test('quota failure makes exactly one request with no fallback', async () => {
  ...
  await withFetch(async () => {
    requests++;
    return Response.json({ error: { code: 'quota_exceeded', message: 'PRIVATE', request_id: 'safe-id' } }, { status: 429 });
  }, async () => {
    await assert.rejects(gatewayChatStream(env, params, () => {}), (error: unknown) => error instanceof AssistantError && error.httpStatus === 503 && error.message === COACH_DOWN);
...
for (const [label, body] of [
  ...
  test(`${label} preserves safe response correlation`, async () => {
    await withFetch(async () => new Response(body, { headers: { 'x-cail-request-id': 'safe-response-id' } }), async () => {
      await assert.rejects(gatewayChatStream(env, params, () => {}), (error: unknown) =>
        error instanceof AssistantError && error.requestId === 'safe-response-id' && error.message === COACH_DOWN);
    });
  });
}
