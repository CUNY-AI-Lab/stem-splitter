"""Strict, dependency-light contract checks for the EfficientAT comparator."""
...
def load_class_map(path: Path) -> tuple[AudioSetClass, ...]:
    raw = _read_pinned(path, CLASS_MAP_SHA256, "EfficientAT class map")
    if len(raw) != CLASS_MAP_BYTES:
        raise ComparatorContractError("EfficientAT class map byte length does not match")
    try:
        text = raw.decode("utf-8")
        rows = list(csv.DictReader(text.splitlines()))
    except (UnicodeDecodeError, csv.Error) as error:
        raise ComparatorContractError("EfficientAT class map is invalid") from error
    if len(rows) != CLASS_COUNT or set(rows[0] if rows else {}) != {
        "index",
        "mid",
        "display_name",
    }:
        raise ComparatorContractError("EfficientAT class map schema does not match")
    ...
    for expected_index, row in enumerate(rows):
        try:
            ...
            raise ComparatorContractError("EfficientAT class index is invalid") from error
        mid = row.get("mid")
        ...
        if (
            ...
            raise ComparatorContractError("EfficientAT class map row is invalid")
        seen_mids.add(mid)
...
def load_mapping(
    ...
    raw = _read_pinned(mapping_path, MAPPING_SHA256, "EfficientAT candidate mapping")
    try:
        document = json.loads(raw.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise ComparatorContractError("EfficientAT candidate mapping is invalid") from error
    if not isinstance(document, dict):
        raise ComparatorContractError("EfficientAT candidate mapping root is invalid")
    _exact_keys(
        document,
        {
            "$schema",
    ...
    if (
        ...
        raise ComparatorContractError("EfficientAT candidate mapping identity does not match")
    policy = document["scoringPolicy"]
    if not isinstance(policy, dict):
        raise ComparatorContractError("EfficientAT scoring policy is invalid")
    _exact_keys(
        policy,
        {
            "classAggregation",
    ...
    } or SCORING_POLICY_VERSION != "single-clip-sigmoid-second-window-v1":
        raise ComparatorContractError("EfficientAT scoring policy does not match the code pin")
    ...
    if not isinstance(raw_mapped, list) or not isinstance(raw_unsupported, list):
        raise ComparatorContractError("EfficientAT mapping lists are invalid")
    ...
    for item in raw_mapped:
        if not isinstance(item, dict):
            raise ComparatorContractError("EfficientAT mapped instrument is invalid")
        _exact_keys(item, {"instrumentId", "classes"}, "EfficientAT mapped instrument")
        ...
        if (
            ...
            raise ComparatorContractError("EfficientAT mapped instrument identity is invalid")
        classes: list[AudioSetClass] = []
        for class_value in classes_value:
            if not isinstance(class_value, dict):
                raise ComparatorContractError("EfficientAT mapped class is invalid")
            _exact_keys(class_value, {"index", "mid", "displayName"}, "EfficientAT mapped class")
            ...
            if (
                ...
                raise ComparatorContractError("EfficientAT mapped class identity is invalid")
            ...
            if class_map is not None and class_map[index] != candidate:
                raise ComparatorContractError("EfficientAT mapped class does not match the class map")
    ...
    for item in raw_unsupported:
        if not isinstance(item, dict):
            raise ComparatorContractError("EfficientAT unsupported instrument is invalid")
        _exact_keys(item, {"instrumentId", "reason"}, "EfficientAT unsupported instrument")
        instrument_id = item["instrumentId"]
        reason = item["reason"]
        if (
            not isinstance(instrument_id, str)
            ...
            raise ComparatorContractError("EfficientAT unsupported instrument identity is invalid")
        seen_ids.add(instrument_id)
        unsupported.append(UnsupportedInstrument(instrument_id, reason))

    if seen_ids != set(vocabulary_ids):
        raise ComparatorContractError("EfficientAT mapping does not partition the vocabulary")
    ...
    if [item.instrument_id for item in instruments] != [
        ...
        raise ComparatorContractError("EfficientAT mapped instruments are not in vocabulary order")
    if [item.instrument_id for item in unsupported] != [
        item for item in vocabulary_ids if item not in mapped_ids
    ]:
        raise ComparatorContractError("EfficientAT unsupported instruments are not in vocabulary order")
...
def decode_pcm_windows(payload: bytes, counts: Sequence[int]) -> tuple[memoryview, ...]:
    if sys.byteorder != "little":
        raise ComparatorContractError("EfficientAT comparator requires little-endian PCM")
...
def summarize_clip_values(values: Sequence[float]) -> dict[str, Union[float, int]]:
    converted = [float(value) for value in values]
    if len(converted) != 1 or any(
        not math.isfinite(value) or value < 0 or value > 1 for value in converted
    ):
        raise ComparatorContractError("EfficientAT clip score is invalid")
    value = converted[0]
    return {
        "top3Mean": value,
        "maximum": value,
        "mean": value,
        "patchesAtLeastHalf": int(value >= 0.5),
    }
