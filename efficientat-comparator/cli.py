#!/usr/bin/env python3
"""Bounded stdin/stdout interface for offline EfficientAT evaluation."""
...
from backend import EfficientatBackend
...
def arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="offline EfficientAT comparator")
...
def main() -> int:
    ...
    try:
        ...
        backend = EfficientatBackend(
            Path("/models/efficientat"),
        ...
        if len(encoded) > MAX_OUTPUT_BYTES:
            raise ComparatorContractError("EfficientAT comparator output exceeds the limit")
        ...
        print("EfficientAT comparator failed", file=sys.stderr)
        return 1
