...
2b. **YouTube import:** `POST /api/jobs` with `{ youtubeUrl }` instead of `{ key, filename }` fetches audio in-Worker via `youtubei.js` (`src/youtube.ts`, behind `fetchYouTubeAudio()` so it can be swapped for an external fetcher), stores it at `uploads/<uuid>/source.m4a`, then proceeds like a normal job. 15-min cap, no live streams; the job row is created only after audio lands in R2, so failed fetches never leave stuck jobs. **Current status: YouTube bot-checks all requests from Cloudflare egress IPs** ("Sign in to confirm you're not a bot") — the code tries a client fallback chain (IOS/ANDROID/TV/…) and works from residential IPs, but in production the fetch currently fails with a clean, user-readable error. Known escape hatches: a logged-in-session cookie secret, or an external fetcher (Replicate yt-dlp) behind the same seam. File uploads are unaffected.
