...
**Frontend** (`public/`, vanilla JS, no build step): one page, two workshop stations behind upper-left tabs (`role=tablist`; panels flip `hidden`, nothing tears down). **01 · Splitter:** the `Mixer` class in `app.js` plays all stems as parallel `HTMLAudio` elements — first stem is the master clock, others are nudged back if drift exceeds 80 ms (500 ms interval). Job list lives in localStorage; `mixers` Map preserves player state across re-renders. **02 · Remixer** (spec: `docs/superpowers/specs/2026-09-01-splitter-remixer-stations-design.md`): the reverse flow — the shelf lists finished splits as pullable layers, the deck stacks them through gain→pan→master Web Audio chains with per-layer entry/speed/tape/loop/reverse (decoded sample-reversed buffers, 10-min cap) and a `MediaRecorder` CAPTURE bounce; the Crate lives here (same ids/routes — imports still run as normal jobs and surface on the shelf), and Listening Guy's devil's-advocate panel is a server-side prompt register: `mode: 'remix'` on the per-job chat endpoint fences the request's `deck` snapshot into the prompt as data and narrows tools to solo/set_mute (offer and sanitizer both). Remixer classes deliberately avoid `badge`/`channel`/`coach-*`/`play-btn`, which e2e specs match strictly; crate specs click the REMIXER tab first. Visual language is a "studio console" theme (per-stem channel colors are CSS vars `--c-vocals` etc. in `styles.css`).
