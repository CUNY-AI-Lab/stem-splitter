...
**Frontend** (`public/`, vanilla JS, no build step): Splitter and Remixer are separate tabs behind `REMIXER_ENABLED=false`. The original Splitter mixer and Listening Guy are unchanged. The Cloudflare Remixer starts with the single Crate: PREVIEW and ADD TO MIX fetch reviewed, bounded whole-track audio through authenticated `POST /api/remix/archive-audio`, without a separator call or durable copy. SPLIT remains optional; completed stems can join from the shelf. Up to eight layers support volume, pan, entry, speed, tape, reverse, loop, and mute. Named recordings have a bounded duration and download in a ZIP with audio, attribution and arrangement metadata; takes survive clearing layers. Current design and remaining UX stages: `docs/superpowers/specs/2026-09-07-cloudflare-crate-remixer.md`. Railway deployments/configuration are out of scope.
