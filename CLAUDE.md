...
2. `POST /api/jobs` → row in D1 (`schema.sql`), then `SeparationBackend.start()` with a presigned GET to the source and a webhook URL containing `?job=<id>&token=<WEBHOOK_SECRET>`.
