...
bun run db:migrate        # apply schema.sql to remote D1 (fresh install; additive migrations currently run through db:migrate:13)
...
2a. **Authoritative Auto source handoff:** an uploaded source is streamed into the app-owned `auto-inputs/v1/<job>` key before analysis. Browser PUT routes cannot address that prefix, and both the private analyzer and separator receive signed URLs for that exact snapshot. YouTube and Archive imports instead carry the app-calculated pre-storage SHA-256/byte count into the analyzer identity check. Never point either authoritative path back at a still-live browser upload locator. The app-owned Auto key is a valid later teacher-isolation source; widening that boundary beyond exact `uploads/` and `auto-inputs/v1/<job>` shapes is forbidden.
