...
- The reviewed committed lineage now reaches teacher-governance commit
  ...
  `10f6b0a`, the isolation source guard to `15e782a`, and the shared analyzer
  source-scope fix to `86cd50b`. The role-v4 codec-parity composition and latest
  complete clean committed-source gate are bound to `8901902`.
...
| Phase 1 calibration | A broad “four or six is fine” range hid taxonomy failures. The first strict v1 run under-routed jazz to two and over-routed synthwave to six. | Targets partition preferred, accepted, and rejected outcomes. V2 fixed jazz; v3 adds independent electronic controls and raises the six-track evidence threshold without relabeling the original failure acceptable; v4 prevents one short codec-boundary peak from manufacturing repeating-attack evidence. | Local FFmpeg and real Chrome v4 are 11/11 accepted with complete decision agreement. The pinned 8.0.3 amd64 image remains v3-only; current image, native Railway, and listening gates remain. |
| Cross-service version drift | Merely requiring a non-floating classifier string allowed a separately deployed analyzer or stale browser to route with an unknown version. | The shared contract pins `autosplit-role-v4` exactly. An analyzer mismatch becomes `analysis_contract_invalid` and uses the frozen fallback; an invalid browser comparison summary is rejected before it can affect paid routing. | Parser and paid-routing fallback tests pass. |
...
| Analyzer source-scope integration | The app correctly signed an immutable `auto-inputs/v1/<job>` upload snapshot, but the real analyzer allowlist accepted only `uploads/...`; mocked E2E therefore hid a deployed fallback. Widening the service ad hoc could instead expose stems, isolation inputs, or arbitrary app routes. | One shared `analysis-source-scope-v2` contract permits exact three-segment stored sources and exact v1 Auto snapshots, binds the latter to upload source type, makes the app refuse every other analyzer key, and exposes the compiled pin from `/readyz`. The image smoke must analyze a real Auto-snapshot path. | Exact commit `86cd50b` passes the clean frozen Phase 0 gate: all three typechecks, 183/23/28/5/30/9 code-service tests, and 19/6/1 browser journeys. The canonical read-only Railway pre-provision gate passes with analyzer absent and feature posture off. Native image and Railway reproduction remain required. |
| Real analyzer composition and codec parity | Mocked authoritative E2E proved app ordering and fallback but never decoded the app's signed stored source. The first real composition run found that equivalent WAV upload/Archive sources chose two tracks while a YouTube-shaped AAC/M4A transcode chose four because one codec-boundary peak became a short-source onset rate. | A new integration composes the Wrangler app with the real analyzer source policy, FFmpeg decode, classifier, fingerprinting, and cleanup for all three source types. `autosplit-role-v4` requires two refractory-separated onset peaks before onset-derived evidence becomes nonzero; all other thresholds and 2/4/6 contracts remain fixed. | Exact commit `8901902` passes the clean frozen Phase 0 gate: all three typechecks, 184/24/28/5/30/9 code-service tests, and 19/6/1 browser journeys. The real composition returns three non-degraded v4 decisions and concrete two-stem runner inputs; local FFmpeg and real Chrome agree on all 11 corpus choices at 8 preferred, 3 accepted alternatives, 0 mismatches. Current image, native CI, listening, and Railway gates remain. |
...
| Phase 1A source authority | Origin allowlisting alone allowed the analyzer token to fetch arbitrary endpoints on the app origin, and accepting the six-hour separator URL would widen analyzer authority beyond its purpose-specific URL. The first narrow allowlist then omitted the immutable Auto snapshot used by authoritative uploads. | The app and analyzer share `analysis-source-scope-v2`: only exact signed canonical `uploads/<id>/<file>` paths and upload-bound `auto-inputs/v1/<job>` snapshots fit the ten-minute analyzer boundary. Redirects, output/isolation paths, noncanonical encodings, oversized streams, and temporary-file leakage are rejected. | Exact `86cd50b` app/service regressions and clean gate pass. Revised image and Railway execution remain open. |
...
The subsequent cross-service source audit is bound to exact executable-source
commit `86cd50b`. A clean detached checkout installed the frozen Bun 1.3.14
graph and passed the literal `test:phase0` command: all three typechecks; 183
worker, 23 analyzer, 28 Railway server/migration, 5 separator, 30 discovery,
and 9 YAMNet tests; plus 19 flags-off, 6 authoritative-Auto, and 1
isolation-shadow browser journey. A value-free pre-provision audit against the
explicit canonical Railway IDs also passed with `audio-analysis` absent,
feature posture off, zero mutations, zero provider calls, and no secrets
printed. The revised analyzer image smoke did not run because the installed
Docker client had no running daemon; native image and Railway execution remain
open acceptance gates.

The codec-parity follow-up is bound to exact executable-source commit
`8901902`. A fresh detached checkout installed the frozen Bun 1.3.14 graph,
remained clean, and passed the literal `test:phase0` command: all three
typechecks; 184 worker, 24 analyzer, 28 Railway server/migration, 5 separator,
30 discovery, and 9 YAMNet tests; plus 19 flags-off, 6 authoritative-Auto, and
1 isolation-shadow browser journey. The additional analyzer case composes the
real app and analyzer across stored upload, YouTube AAC/M4A, and Archive bytes
and proves analysis precedes each concrete separator request.
`git show --check` also passes. Exact HEAD separately repeated the local
eleven-source corpus at 8 preferred, 3 accepted alternatives, and 0 mismatches
and real Chrome at 11/11 decision agreement. No Railway service, variable,
provider call,
migration, remote branch, pull request, or deployment changed.
...
1. Review the clean `8901902` release scope, then publish the v3.2 branch for
   native GitHub CI when authorized. No pull request or remote branch exists yet.
2. Reproduce the revised role-v4/source-scope-v2 image, runtime allowlist,
   readiness, authentication, and corpus evidence on native CI/amd64; then validate actual
