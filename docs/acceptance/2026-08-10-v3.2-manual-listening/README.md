...
No `review.json` is committed yet. Its absence is intentional: automated hash,
frame, and contract checks cannot assert that a stem sounds usable.
...
Only after that command succeeds may a reviewed change add the same JSON at
`docs/acceptance/2026-08-10-v3.2-manual-listening/review.json`, set
`manualListening` to `true`, remove `manual-listening-missing`, and rerun the
