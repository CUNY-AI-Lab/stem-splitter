...
export async function fleetConfiguration(env: Env) {
  ...
  try {
    ...
    endpoint = !url.username && !url.password && !url.search && !url.hash &&
      ((url.origin === 'https://tools.ailab.gc.cuny.edu' && url.pathname === '/v1') || (env.LOCAL_DEV === '1' && url.protocol === 'http:' && ['127.0.0.1', 'localhost'].includes(url.hostname)));
  } catch { /* Missing or invalid configuration fails closed. */ }
  if (!app.ok || !gateway.ok || !endpoint || !env.ASSISTANT_MODEL || !/^[a-f0-9]{40}$/.test(env.CAIL_SOURCE_VERSION ?? '')) return null;
