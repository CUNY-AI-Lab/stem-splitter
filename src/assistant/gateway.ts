...
export const COACH_QUOTA = 'The CAIL model usage limit has been reached. The mixer still works.';
...
export class AssistantError extends Error {
  ...
  code: string;
  shouldRetry?: boolean;
  constructor(httpStatus: 502 | 503, studentMessage: string, requestId?: string, code = 'assistant_unavailable', shouldRetry?: boolean) {
...
function safeError(error: unknown, fallbackRequestId?: string): AssistantError {
  ...
  const requestId = typeof id === 'string' && REQUEST_ID.test(id) ? id : undefined;
  ...
  const message = code === 'quota_exceeded' ? COACH_QUOTA : shouldRetry === false
    ? 'The Listening Guide could not complete this request. The mixer still works.' : COACH_DOWN;
  return new AssistantError(error instanceof CailError && error.status === 429 ? 503 : 502, message, requestId ?? fallbackRequestId, code, shouldRetry);
}
...
export async function gatewayChatStream(
  ...
  try {
    ...
    const rawRequestId = response.headers.get('x-cail-request-id') ?? response.headers.get('x-request-id');
    requestId = rawRequestId && REQUEST_ID.test(rawRequestId) ? rawRequestId : undefined;
