...
import { buildGuideInstruction, buildSystemPrompt, fmtTime, SYSTEM_PROMPT_VERSION } from './prompt';
...
export async function getGuide(env: Env, jobId: string): Promise<GuideRecord | null> {
  const row = await env.DB.prepare(
    `SELECT guides.text, guides.model, guides.created_at
     FROM guides
 ...
 * it is still current. The prompt identity travels with the row so a fixed
 * code-prompt release cannot reuse a guide generated by an older version.
 *
...
export async function cacheGuideIfPromptCurrent(
  ...
  const result = await env.DB.prepare(
    `INSERT INTO guides
       (job_id, text, model, created_at, prompt_version, prompt_revision)
     SELECT ?, ?, ?, ?, ?, ?
...
export async function streamGuide(
  ...
  await cacheGuideIfPromptCurrent(env, guide, promptState.revision);
